...
let updateProgressFn: ((uploadId: string, bytesUploaded: number, totalBytes: number, status: string, message?: string) => void) | null = null;
...
export class StreamingUploadHandler {
    ...
    async handleStreamingUpload(
        ...
        console.log(`Starting streaming upload for ${filename} (${Math.round(contentLength / 1024 / 1024)} MB)`);
        ...
        try {
            while (totalBytesWritten < contentLength) {
                ...
                if (totalBytesWritten - lastProgressLog > 100 * 1024 * 1024) {
                    ...
                    if (contentLength > 0) {
                        const percent = Math.round((totalBytesWritten / contentLength) * 100);
                        console.log(`Upload progress: ${progressMB}MB / ${totalMB}MB (${percent}%)`);
                    } else {
            ...
            console.log(`Upload completed: ${Math.round(totalBytesWritten / 1024 / 1024)} MB written`);
