...
export class EnvironmentValidator {
  ...
  static validateProduction(): { valid: boolean; errors: string[] } {
    ...
    if (!config.r2AccountId) errors.push('CLOUDFLARE_R2_ACCOUNT_ID is required');
    if (!config.r2AccessKey) errors.push('CLOUDFLARE_R2_ACCESS_KEY is required');
    if (!config.r2SecretKey) errors.push('CLOUDFLARE_R2_SECRET_KEY is required');
  ...
  static validateDevelopment(): { valid: boolean; warnings: string[] } {
    ...
    if (!this.isR2Configured()) {
      warnings.push('R2 configuration not found - backup functionality will be disabled');
    }

    if (!config.adminPassword) {
      warnings.push('ADMIN_PASSWORD not set - using default development password');
    }
