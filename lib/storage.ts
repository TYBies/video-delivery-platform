...
export class LocalStorage {
     ...
     */
    async getVideoStream(videoId: string): Promise<{ stream: NodeJS.ReadableStream; size: number; filename: string }> {
        const videoDir = path.join(this.storagePath, 'videos', videoId);
        const files = await fs.readdir(videoDir);
        const videoFile = files.find(file => file.startsWith('video.'));
     ...
     */
    async getStorageStats(): Promise<{ totalSize: number; videoCount: number; freeSpace?: number }> {
