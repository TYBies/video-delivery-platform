...
export function loadS3Config(): S3EnvConfig {
  ...
  if (!endpoint)
    throw new Error(
      'Storage endpoint missing: set B2_S3_ENDPOINT or S3_ENDPOINT'
    );
  if (!bucket) throw new Error('Bucket missing: set B2_BUCKET or S3_BUCKET');
  if (!accessKeyId || !secretAccessKey)
    throw new Error(
      'Access keys missing: set B2_KEY_ID/B2_APPLICATION_KEY (or S3_ACCESS_KEY_ID/S3_SECRET_ACCESS_KEY)'
    );
...
export function handleS3Error(error: unknown): {
  message: string;
  userFriendly: string;
  isRateLimited: boolean;
} {
  const err = (error ?? {}) as AwsLikeError & Error;
  const errorMessage = err.message || '';
  const errorCode = err.Code || err.$metadata?.httpStatusCode;

  if (
    err.Code === 'AccessDenied' &&
    errorMessage.includes('bandwidth or transaction')
  ) {
    return {
  ...
  if (err.Code === 'AccessDenied' && errorMessage.includes('cap exceeded')) {
    return {
      message: 'Daily transaction cap exceeded',
      userFriendly:
        'Daily cloud storage transaction limit reached. This will reset at midnight GMT. Please try again later.',
      isRateLimited: true,
    };
  }
  ...
  if (err.Code === 'AccessDenied') {
    return {
      ...
      userFriendly:
        'Access to cloud storage was denied. This may be due to daily limits that reset at midnight GMT or configuration issues. Please try again later.',
      isRateLimited: true,
    };
  }

  if (err.Code === 'NoSuchKey' || errorCode === 404) {
    return {
  ...
  return {
    ...
    userFriendly:
      'Cloud storage is temporarily unavailable. Please try again later.',
    isRateLimited: false,
  };
}
