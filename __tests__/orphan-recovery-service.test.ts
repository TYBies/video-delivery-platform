...
describe('OrphanRecoveryService', () => {
  ...
  const createTestOrphanFile = async (videoId: string, filename: string = 'video.mp4'): Promise<OrphanFile> => {
  ...
  const createTestVideoWithMetadata = async (videoId: string): Promise<void> => {
    const videoDir = path.join(testStoragePath, 'videos', videoId);
    await fs.mkdir(videoDir, { recursive: true });
  ...
  describe('reconstructMetadata', () => {
    it('should reconstruct metadata from orphan file', async () => {
      const orphan = await createTestOrphanFile('test-orphan', 'client-project-video.mp4');
  ...
  describe('recoverOrphan', () => {
    it('should successfully recover valid orphan', async () => {
      const orphan = await createTestOrphanFile('recoverable-orphan', 'test-video.mp4');
      ...
      const metadataPath = path.join(testStoragePath, 'videos', 'recoverable-orphan', 'metadata.json');
      const exists = await fs.access(metadataPath).then(() => true).catch(() => false);
      expect(exists).toBe(true);
    });

    it('should fail to recover invalid orphan', async () => {
      const orphan = await createTestOrphanFile('invalid-orphan', 'document.txt');
    ...
    it('should update orphan registry on recovery', async () => {
      ...
      expect(registry.orphans['registry-test'].recoveryStatus).toBe('recovered');
  ...
  describe('cleanupInvalidOrphans', () => {
    ...
    it('should not affect valid orphans', async () => {
      ...
      const videoPath = path.join(testStoragePath, 'videos', 'valid-orphan', 'video.mp4');
