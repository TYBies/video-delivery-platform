...
async function simulateUploadRequest(videoId: string): Promise<void> {
...
async function main() {
  ...
  try {
    ...
    const orphanId = await createOrphanedFile();
    await simulateUploadRequest(orphanId);
    
    // Test 2: Background recovery service
    console.log('\n📋 Test 2: Background recovery service');
    
    // Create another orphaned file
    const orphanId2 = await createOrphanedFile();
